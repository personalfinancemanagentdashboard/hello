...
if (process.env.OPENAI_API_KEY) {
  ...
  console.warn("OPENAI_API_KEY not set. AI chat features will be disabled.");
}
  ...
  app.post("/api/ai/chat", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const recentTransactions = transactions
        .slice(0, 10)
        .map((t) => `  - ${t.date}: ${t.title} (${t.category}) - $${parseFloat(t.amount).toFixed(2)} [${t.type}]`)
        .join('\n') || '  No transactions yet';

      const systemMessage = {
        role: "system",
        content: `You are SmartFinance.AI, a helpful personal finance assistant. Analyze the user's actual financial data and provide clear, actionable advice.
...
Monthly Comparison:
  Current Month Spending: $${currentMonthExpenses.toFixed(2)}
  Last Month Spending: $${lastMonthExpenses.toFixed(2)}
  Change: ${currentMonthExpenses > lastMonthExpenses ? '+' : ''}$${(currentMonthExpenses - lastMonthExpenses).toFixed(2)}
...
3. Structure responses with clear sections (use line breaks)
4. Keep it conversational and easy to understand
5. Provide actionable tips based on their spending patterns
6. NO markdown symbols - plain text only

Example good response format:

Your Spending This Month 📊

You spent $1,234.56 this month across these categories:
- Food: $450.00
- Transportation: $300.00
- Entertainment: $200.00

Compared to last month, you're spending $100 more.

Here's my recommendation 💡
- Your food spending is high. Try meal planning to save $100-150 monthly.
- Consider budgeting $400 for food next month.
